...
static count_map_t count_words_parallel(word_t *words, size_t num_words) {
  ...
  for (size_t i = 0; i < THREAD_COUNT; i++) {
    ...
    add_word_counts_in_chunk(&map, thread_arg_words, thread_arg_num_words,
                             &count_mutex);
    pthread_create(&threads[i], NULL, counter_thread_func,
                   create_entry(*thread_arg_words, thread_arg_num_words));
  }
